...
const requestSchema = z.object({
    body: z.array(
        z.object({
            id: z.string(),
            type: z.string(),
            dataset: z.object({}).passthrough(),
            internal: z.object({}).passthrough(),
        })
    ),
});

export const uploadAnalyses = endpointErrorHandler(async (request: Request, response: Response, next: NextFunction) => {
    ...
    if (insertResult.insertedCount > 0) {
        response.status(200).send({ inserted: insertResult.insertedCount });
    } else {
