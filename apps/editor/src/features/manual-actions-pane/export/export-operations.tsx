...
export type IncludeInfo = { transformations: boolean; importedData: boolean; prefixes: boolean; diagramPositions: boolean };
...
export function ExportOperations() {
    ...
    const handleDataTransformationsCheckChange = () => dispatchInclude({ type: 'include-data-transformations' });
    ...
    const handleDiagramPositionsCheckChange = () => dispatchInclude({ type: 'include-diagram-positions' });
    ...
    const exportOperations = () => {
        const importOperations = history.operations
            .map((operation, index) => (operation.type === 'import-schema-and-instances' ? { dataImport: true, index } : { dataImport: false }))
            .filter(({ dataImport }) => dataImport);

        const relevantOperations = history.operations.slice(importOperations.at(-1)?.index ?? 0);

        const includeDataTransformations = (operation: UpdateOperation) =>
            operation.type !== 'transform-schema-and-instances' || include.transformations;
        const includeInitialData = (operation: UpdateOperation) => operation.type !== 'import-schema-and-instances' || include.importedData;
        const includeDiagramPositions = (operation: UpdateOperation) => !isDiagramUpdateOperation(operation) || include.diagramPositions;
        ...
        const file = new File([JSON.stringify(exportData)], 'operations.json', { type: 'application/json' });
        download(file);
        ...
        <div>
            ...
            <Dropdown headerLabel='Include' showInitially>
                <div className='mx-2 flex flex-col gap-1'>
                    ...
                    <label className='p-1 bg-slate-300 rounded'>
                        <input className='p-1 mr-1' type='checkbox' checked={include.importedData} onChange={handleImportedDataCheckChange} />
                        Imported Data
                    </label>
                    <label className='p-1 bg-slate-300 rounded'>
                        <input className='p-1 mr-1' type='checkbox' checked={include.prefixes} onChange={handlePrefixesCheckChange} />
                        Prefixes
                    </label>
                    <label className='p-1 bg-slate-300 rounded'>
                        <input className='p-1 mr-1' type='checkbox' checked={include.diagramPositions} onChange={handleDiagramPositionsCheckChange} />
                        Diagram Positions
                    </label>
                </div>
            </Dropdown>
